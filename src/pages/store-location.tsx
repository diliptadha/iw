    ...
    <>
      <div className=" px-[2rem] py-[2rem] md:px-[3rem] xl:px-[6rem]">
        {data.store.length > 0 ? (
          data.store.map((ele) =>
          (
            <div className="border rounded-[10px] lg:max-w-[80%] md:max-w-[90%] xl:max-w-[70%] md:my-0 md:mx-auto flex-wrap md:flex-none text-justify	tracking-[0.5px] text-[12px] md:text-[15px] p-[20px] flex">
              ...
              <div className="flex-1">
                <div className="mb-[15px] font-bold flex justify-between items-center ">
                  <h2 className="text-[16px] md:text-[18px]">{ele.name}</h2>
                  <div className="flex items-center gap-[2px]">
                    <StarRating rating={ele.rating} />
                  </div>
                </div>
                <h3 className="mb-[6px] md:mb-[16px]">{ele.address}</h3>
                <h4 className="mb-[10px] md:mb-[20px] md:text-[15px]">{ele.timing}</h4>
                <div className="flex gap-[10px] mb-[20px] items-center">
                  ...
                  <Link href={`tel:-${ele.number}`} className="underline text-[#000000] hover:text-[#42b7e9]">+91 {ele.number}</Link>
                </div>
