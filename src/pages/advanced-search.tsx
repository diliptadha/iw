...
const Listingpage: React.FC<{ filters: Filters }> = ({ filters }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      console.log("csdfbsdhvdsfgusdgsdvsvgssdy", JSON.stringify(response.data));
  ...
  const fetchFilterData = async () => {
    try {
      ...
      let config = {
        method: "get",
        maxBodyLength: Infinity,
        url: `http://localhost:4000/product/getFilterProductData?gender=${urlSelectedGender}&sortBy=${sortBy}&page=1&limit=9`,
        headers: {
  ...
  const pageCountFilter = Math.ceil(
    (filterData.length || productList.length) / perPage
  );
  ...
  const fetchTrendingSearch = async (search: string) => {
    try {
      const config = {
        method: "get",
        maxBodyLength: Infinity,
        url: `http://localhost:4000/home/trendingSearch?term=${search}`,
        headers: {},
      };
  ...
  // const handlePageChangeFilter = (selected: any) => {
  //   setCurrentPageFilter(selected);
  // };

  // const handlePageChangeTrending = (selected: any) => {
  //   setCurrentPageTrending(selected);
  // };

  // const slicedDataFilter = (filterApplied ? filterData : productList).slice(
  //   currentPageFilter * perPage,
  //   (currentPageFilter + 1) * perPage
  // );

  // const slicedDataTrending = trendingSearchData.slice(
  //   currentPageTrending * perPage,
  //   (currentPageTrending + 1) * perPage
  // );
  return (
    <div className="list-bg max-w-screen-2xl m-auto">
      ...
      <div className="mt-[36px] xs:mx-[20px] xl:mx-[72px]- mx flex">
        ...
        <div className="md:ml-3 xl:ml-7 w-full">
          <div className="flex items-center xs:justify-center md:justify-between xs:flex-col lg:flex-row xs:space-y-2 lg:space-y-0 text-black font-normal text-sm">
            ...
            <p className="xs:order-3 lg:order-2">
              {search === ""
                ...
                : `Showing ${trendingSearchData.length} of ${trendingSearchData.length} results`}
            </p>
          ...
          <div
            className={`mt-7 md:mx-5 xl:mx-0 flex flex-wrap xs:justify-center 
                 lg:justify-start lg:gap-x-3 xl:gap-x-9 
